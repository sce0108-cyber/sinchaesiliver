...
`--benchmark` 옵션으로 측정했습니다. 과제에서 하라는 대로 127.0.0.1 1-1000 포트로 먼저 재봤는데, 순차나 스레드나 거의 차이가 없었다. 그래서 filtered 포트가 섞여있는 scanme.nmap.org로도 한 번 더 재봤다.
...
**분석:** (초안 — 제출 전 본인 말로 다듬을 것)

127.0.0.1에서는 순차와 스레드 속도 차이가 거의 없었다. 닫힌 포트라도 로컬에서는 RST 응답이 즉시(거의 0초) 오기 때문에, 애초에 기다릴 게 없어서 스레드로 동시에 돌리는 이점이 안 나타난 것. 반면 scanme.nmap.org는 방화벽에 막힌(filtered) 포트가 섞여있어서, 순차 스캔은 그런 포트마다 timeout(1초)을 꽉 채워 기다려야 했고 그게 200번 누적되니 32초가 걸렸다. 스레드로 돌리면 그 대기 시간들이 겹쳐서 진행되기 때문에 1.42초로 끝났다. 결국 스레드의 이득은 "응답이 느리거나 없는 대상"을 스캔할 때 확실히 드러난다.
...
- 열린 포트일 때: SYN → SYN/ACK → ACK 까지 3단계가 다 일어난다. `connect_ex()`가 0을 반환하는 시점이 ACK까지 보낸 뒤다.
- 닫힌 포트일 때: 상대가 SYN을 받자마자 RST로 바로 거절해서, handshake가 1단계(SYN)만 가고 끝난다. ACK 단계까지 못 감.
- 방화벽이 막았을 때: SYN을 보내도 응답 자체가 안 온다(드롭). handshake가 아예 시작도 못 하고, 코드에서는 그냥 `timeout` 시간만큼 기다리다가 실패로 처리된다.

### 2. `connect_ex`와 `connect`의 차이는? 왜 스캐너에는 `connect_ex`가 적합한가?

(초안 — 제출 전 본인 말로 다듬을 것)

`connect()`는 연결에 실패하면 예외(exception)를 던져서, 포트마다 try/except로 감싸야 한다. 반면 `connect_ex()`는 예외 대신 결과를 정수로 반환한다 — 성공하면 0, 실패하면 errno 값. 포트 스캐너는 수백~수천 개 포트를 반복문으로 돌려야 하는데, 그때마다 예외 처리를 하는 것보다 반환값만 비교하는 게 코드도 간단하고 오버헤드도 적어서 `connect_ex`가 더 적합하다.

### 3. 스레드를 너무 많이 띄우면 어떤 문제가 생기는가?

(초안 — 제출 전 본인 말로 다듬을 것)

- 스레드마다 스택 메모리를 OS가 할당하기 때문에, 수천 개씩 띄우면 메모리 사용량이 커진다.
- 소켓도 스레드 수만큼 동시에 열리는데, OS가 프로세스당 열 수 있는 파일 디스크립터 개수를 제한해놔서 `Too many open files` 같은 에러가 날 수 있다.
- 스레드가 너무 많아지면 컨텍스트 스위칭 비용이 늘어나서, 어느 지점부터는 스레드를 늘려도 성능이 오히려 정체되거나 떨어진다.
- 짧은 시간에 연결 시도가 몰리기 때문에 대상 서버의 방화벽이나 IDS가 이상 트래픽(포트 스캔)으로 탐지하고 차단할 수도 있다.
